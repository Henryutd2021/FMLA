# model 3 new varable Big M method model for buying and selling not at the same time (works)
...
var bsf{factory, 1..J, 1..T} binary;
var bbf{factory, 1..J, 1..T} binary;
var bsn{warehouse, 1..J, 1..T} binary;
var bbn{warehouse, 1..J, 1..T} binary;
...
subject to c_1{k in factory, j in 1..J, t in 1..T}:sum{i in production, n in warehouse}(E[i]+qv*N[k, n]*dis[k, n]*m[i])*xt[i, k, n, j]
           +qv*(sum{n in warehouse, i in production}N[k, n]*dis[k, n]*wv*v[k, n, i, j])+Bk[k, j, t]-Bk[k, j, t-1]+Esk[k, j, t]-Ebk[k, j, t]
           =sum{g in renewables}taog[g]*Pcgk[k, g]*¦Ëk[j, t, k, g];
           
subject to c_2{n in warehouse, j in 1..J, t in 1..T}:tw[n]*Ln[n]*u1[n]+qv*(sum{k in factory, i in production}N[k, n]*dis[k, n]*v[k, n, i, j]*wv)
           +qv*(sum{i in production, s in store}dis1[n, s]*m[i]*xt1[i, n, s, j])
           +qv*2*(sum{s in store, i in production}N1[n, s]*dis1[n,s]*wv*v1[n, s, i])+Bn[n, j, t]-Bn[n, j, t-1]+Esn[n, j, t]-Ebn[n, j, t]
           =sum{g in renewables}taog[g]*Pcgn[n, g]*¦Ën[j, t, n, g];


                                                
subject to c_k4{k in factory, j in 1..J, t in 1..T}: Esk[k, j, t]-M*u[k]<=0;                           
subject to c_k41{k in factory, j in 1..J, t in 1..T}: Ebk[k, j, t]-M*u[k]<=0; 

subject to c_n4{n in warehouse, j in 1..J, t in 1..T}: Esn[n, j, t]-M*u1[n]<=0;                                                       
subject to c_n41{n in warehouse, j in 1..J, t in 1..T}: Ebn[n, j, t]-M*u1[n]<=0; 

subj to c_71{g in renewables, k in factory}: Pcgk[k, g]-M*u[k]<=0;
subj to c_72{g in renewables, n in warehouse}: Pcgn[n, g]-M*u1[n]<=0;

subj to c_81{n in warehouse}: Bcn[n]-M*u1[n]<=0;
subj to c_82{k in factory}: Bck[k]-M*u[k]<=0;
...
subj to c_15{k in factory}: Bk[k, 16, 168] = Bck[k];

subj to c_16{n in warehouse}: Bn[n, 16, 168] = Bcn[n];
...
subj to bs_1{k in factory, j in 1..J, t in 1..T}:Ebk[k, j, t] <= pbm*bbf[k, j, t];

subj to bs_2{k in factory, j in 1..J, t in 1..T}:Esk[k, j, t] <= psm*bsf[k, j, t];

subj to bs_3{n in warehouse, j in 1..J, t in 1..T}:Ebn[n, j, t] <= pbm*bbn[n, j, t];

subj to bs_4{n in warehouse, j in 1..J, t in 1..T}:Esn[n, j, t] <= psm*bsn[n, j, t];

subj to bs_5{k in factory, j in 1..J, t in 1..T}:bbf[k, j, t]+bsf[k, j, t] <= 1;

subj to bs_6{n in warehouse, j in 1..J, t in 1..T}:bbn[n, j, t]+bsn[n, j, t] <= 1;
