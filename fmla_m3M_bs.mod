# model 3 new  varable Big M method model (for differnent selling and buying price)
...
minimize total_cost: sum{k in factory}F[k]*u[k]+sum{i in production, j in 1..J, k in factory, n in warehouse}p[i]*x[i, j, k, n]
         ...
         +sum{t in 1..T, j in 1..J, k in factory}(¦Ñb[t]*Ebk[k, j, t] - ¦Ñs[t]*Esk[k, j, t])
         ...
         +sum{t in 1..T, j in 1..J, n in warehouse}(¦Ñb[t]*Ebn[n, j, t] - ¦Ñs[t]*Esn[n, j, t]);
